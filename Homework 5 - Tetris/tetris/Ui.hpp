...
namespace Bastet{
  ...
  public:
    ...
    void RedrawWell(const Well *well, const Block* falling);
    void ClearNext(); //clear the next block display
    void RedrawNext(const Block* next); //redraws the next block display
    void RedrawScore();
    void CompletedLinesAnimation(const LinesCompleted &completed);
    void DropBlock(Block* b, Well *w); //returns <score,lines>
