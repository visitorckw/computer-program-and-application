...
namespace Bastet{
  ...
  LinesCompleted Well::Lock(Block* b){
    if(b->IsOutOfScreen())
      throw(GameOver());
    ...
    //checks if the 4 lines occupied by the block are completed
    LinesCompleted lc;
    lc._baseY = b->GetBaseY();
    for(int k = 0; k < 4; ++k){
      int l = lc._baseY + k;
  ...
  int Well::LockAndClearLines(Block* b){
    LinesCompleted lc = Lock(b);
    ClearLines(lc);
