...
namespace Bastet{
  ...
  public:
    ...
    LinesCompleted Lock(Block* b); //permanently adds a tetromino to the well; returns a bitset of 4 bits where return[i]==1 iff line (start of fb)+i is complete
    void ClearLines(const LinesCompleted &lc); //removes the given lines from the well (whether they are completed or not)
    int LockAndClearLines(Block* b); //locks, clear lines, returns number of lines cleared
